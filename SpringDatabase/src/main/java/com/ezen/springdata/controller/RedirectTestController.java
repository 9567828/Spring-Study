...
public class RedirectTestController {
	...
	public String home(String fruit, Integer price, Model model) {
		...
		// 어트리리뷰트의 이름(키값)을 설정하지 않고 넣으면
		// 해당 클래스의 이름으로 이름을 자동생성한다 (e.g. Employee -> employee)
		// 생략하고 추가할 때는 절대로 null을 추가할 수 없다
		// (null로는 이름 자동 생성이 불가능)
	...
	public String test1() {
		log.info("테스트1에 도착했따 홈으로갑니다");
		return "redirect:/redirect/";
	}

	@GetMapping("/redirect/test2")
	public String test2() {
		log.info("리다이렉트시 GET방식으로 파라미터 전달하기");
	...
	public String test3(RedirectAttributes rattr) {
		// 리다이렉트시 한 번 사용되고 사라지는 어트리뷰트 (메모리관리가 용이)
